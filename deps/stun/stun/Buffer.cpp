...
Buffer::Buffer(const char* b, size_t n)
: _v(2 * n)
, _max_size(0)
, _read_index(0)
, _write_index(0)
...
ssize_t Buffer::send(SOCKET fd)
{
    ...
    while(readable() > 0)
    {
        ssize_t n = ::send(fd, read(), (int)readable(), 0);
        if(n <= 0)
        {
            break;
        }
...
ssize_t Buffer::sendTo(SOCKET fd, struct sockaddr_storage* to)
{
    ...
    ssize_t n = ::sendto(fd, read(), (int)readable(), 0, reinterpret_cast<struct sockaddr*>(to), tolen);
    if(n > 0)
    {
        read(n);
    }
...
ssize_t Buffer::receive(SOCKET fd)
{
    ...
    {
        reserve(512);
        ssize_t n = ::recv(fd, (char*)write(), (int)writable(), 0);
        if(n <= 0)
        {
            break;
        }
...
ssize_t Buffer::receiveFrom(SOCKET fd, struct sockaddr_storage* from)
{
    ...
    ssize_t n = ::recvfrom(fd, write(), (int)writable(), 0, reinterpret_cast<struct sockaddr*>(from), &fromlen);
    if(n > 0)
    {
        write(n);
    }
...
size_t Buffer::writeBlob(const char* b, size_t n)
{
    assert(writable() >= n);
    memcpy(write(), b, n);
    write(n);
...
bool Buffer::readBlob(char* b, size_t n)
{
    if(readable() < n)
    {
        return false;
