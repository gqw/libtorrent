...
std::string addressToString(const struct sockaddr_in& sin)
{
	...
	char ip[64] = {};
	inet_ntop(AF_INET, &sin.sin_addr, ip, 64);
    oss << ip << ":" << ntohs(sin.sin_port);
    return oss.str();
}
...
ssize_t UdpSocket::write(const char* buf, size_t size)
{
	return ::sendto(_socket, buf, (int)size, 0, (struct sockaddr*)&_sin, sizeof(_sin));
}

ssize_t UdpSocket::read(char* buf, size_t size)
{
    struct sockaddr_in sin;
    memset(&sin, 0, sizeof(sin));
    socklen_t len = sizeof(sin);
	return ::recvfrom(_socket, buf, (int)size, 0, (struct sockaddr*)&sin, &len);
}

ssize_t UdpSocket::read(char* buf, size_t size, int timeout)
{
    struct timeval tv;
